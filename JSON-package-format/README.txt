...
The files that can be contained in an update are:
    ...
    item.json - this defines the metadata and properties for a book/manuscript. e.g. MS-HEBREW-GASTER-00086.json
...
There is a base item schema (schemas/item.schema.json) which defines the structure which all digital library items
will follow, and a specific schema per instance, which allows for variations in properties used for
different institutions. So for Manchester data the mudl.item.schema.json should be used and this extends the base
schema with specific properties to help in data creation.

The schemas are as currently as restrictive as possible to catch any potential problems with data as soon as possible
